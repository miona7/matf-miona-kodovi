...
class XOState:
    ...
    # uracunava broj poteza u evaluaciju tako da se pobedjuje sa najmanjim brojem poteza
    def evaluate(self):
        winner = self.get_winner()
        if winner == "X":
            return 1 / self.move_count # ili 10 - self.move_count
        elif winner == "O":
            return -1 / self.move_count # ili -(10 - self.move_count)
        return 0
    
    # lenjo generisanje stanja
        ...
        for i in range(3):
            for j in range(3):
                if self.board[i][j] == XOState.empty:
                    ...
                    yield next_state
                    # pauzira fju, vrati vrednost (next_state), zapamti gde je stao -> nastavlja pri sledecem pozivu
